...
def run_backtest():
    try:
        # Get form data
        symbol = request.form.get('symbol', 'EURUSD')  # Default to EURUSD if not specified
        # Append 'm' suffix for Exness symbols
        if not symbol.endswith('m'):
            symbol = symbol + 'm'
            
        timeframe = request.form.get('timeframe', 'H1')
        initial_deposit = float(request.form.get('initialDeposit', 10000))
        ...
        if request.form.get('useDate') == 'true':
            start_date = datetime.strptime(request.form.get('startDate'), '%Y-%m-%d')
            end_date = datetime.strptime(request.form.get('endDate'), '%Y-%m-%d')

        print(f"Getting data for {symbol} from {start_date} to {end_date}")
        
        # Get historical data
        rates = mt5.copy_rates_range(symbol, mt5_timeframe, start_date, end_date)
        if rates is None:
            error = mt5.last_error()
            return jsonify({'error': f'Failed to get historical data: {error}'}), 400
        ...
        if len(df) == 0:
            return jsonify({'error': 'No data available for selected period'}), 400
        ...
        dates = [df['time'].iloc[0].strftime('%Y-%m-%d')]
        
        # Get EA parameters
        atr_multiplier = float(request.form.get('ATRMultiplier', 2.0))
        atr_period = int(request.form.get('ATRPeriod', 100))
        lot_size = float(request.form.get('Lot', 0.01))
        take_profit = float(request.form.get('TakeProfit', 250))
        stop_loss = float(request.form.get('StopLoss', 100))
        
        print(f"Using parameters: ATR Mult={atr_multiplier}, ATR Period={atr_period}, Lot={lot_size}, TP={take_profit}, SL={stop_loss}")
        
        # Calculate ATR
        df['ATR'] = df['high'] - df['low']
        df['ATR'] = df['ATR'].rolling(window=atr_period).mean()
        
        # Trading logic based on UT Bot strategy
        open_position = None
        ...
        for i in range(1, len(df)):
            ...
            # Check for signals
            buy_signal = (current_bar['close'] > prev_bar['high'] + atr_multiplier * prev_bar['ATR'])
            sell_signal = (current_bar['close'] < prev_bar['low'] - atr_multiplier * prev_bar['ATR'])
            
            # Close existing position if opposite signal
            if open_position is not None:
                if (open_position['type'] == 'buy' and sell_signal) or \
                   (open_position['type'] == 'sell' and buy_signal):
                    # Calculate profit/loss
                    if open_position['type'] == 'buy':
                        profit = (current_bar['close'] - open_position['price']) * lot_size * 100000
                    else:
                        profit = (open_position['price'] - current_bar['close']) * lot_size * 100000
                    ...
                    trades.append({
                        ...
                        'profit': profit,
                        'date': current_bar['time'].strftime('%Y-%m-%d %H:%M')
                    })
            ...
            if open_position is None:
                if buy_signal:
                    open_position = {
                        ...
                        'sl': current_bar['close'] - stop_loss * mt5.symbol_info(symbol).point,
                        'tp': current_bar['close'] + take_profit * mt5.symbol_info(symbol).point
                    }
                elif sell_signal:
                    open_position = {
                        'type': 'sell',
            ...
            # Update equity curve
            if i % 24 == 0:  # Update daily
                equity_curve.append(equity)
                dates.append(current_bar['time'].strftime('%Y-%m-%d'))
        ...
        results = {
            ...
            'netProfit': balance - initial_deposit,
            'grossProfit': gross_profit,
            'grossLoss': gross_loss,
            'profitFactor': round(profit_factor, 2),
            'totalTrades': total_trades,
            'profitTrades': profitable_trades,
            'lossTrades': total_trades - profitable_trades,
            'winRate': round(win_rate, 2),
            'maxDrawdown': round(max_drawdown, 2),
            'dates': dates,
            'equity': equity_curve,
            'trades': trades[:100]  # Return last 100 trades for display
        }
...
def analyze_ea():
    try:
        ...
        # Define the actual parameters from the UT BOT EA
        ea_info = {
            'name': 'UT BOT EA',
            'inputs': [
                # UT BOT Parameters
                {'name': 'ATRMultiplier', 'type': 'double', 'default': 2.0, 'description': 'Key Value (Changes sensitivity)'},
                {'name': 'ATRPeriod', 'type': 'integer', 'default': 100, 'description': 'ATR Period'},
                
                # Trade Parameters
                {'name': 'TradeType', 'type': 'enum', 'default': 'Both', 'options': ['Buy', 'Sell', 'Both'], 'description': 'Trading Direction'},
                {'name': 'Lot', 'type': 'double', 'default': 0.01, 'description': 'Initial Lot Size'},
                {'name': 'LotMultiplier', 'type': 'double', 'default': 1.5, 'description': 'Lot Multiplier for Recovery'},
                {'name': 'TakeProfit', 'type': 'double', 'default': 250, 'description': 'Take Profit in Points'},
                {'name': 'StopLoss', 'type': 'double', 'default': 100, 'description': 'Stop Loss in Points'},
                {'name': 'TrailingStop', 'type': 'double', 'default': 100, 'description': 'Trailing Stop in Points'},
                
                # Time Parameters
                {'name': 'StartTime', 'type': 'string', 'default': '00:00', 'description': 'Trading Start Time (HH:MM)'},
                {'name': 'EndTime', 'type': 'string', 'default': '24:00', 'description': 'Trading End Time (HH:MM)'},
                
                # Other Parameters
                {'name': 'TradeComment', 'type': 'string', 'default': 'UT', 'description': 'Trade Comment'},
                {'name': 'Magic', 'type': 'integer', 'default': 1851, 'description': 'Magic Number'}
            ]
        }
